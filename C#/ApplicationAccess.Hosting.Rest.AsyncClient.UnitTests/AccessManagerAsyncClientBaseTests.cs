...
namespace ApplicationAccess.Hosting.Rest.AsyncClient.UnitTests
{
    /// <summary>
    /// Unit tests for the ApplicationAccess.Hosting.Rest.AsyncClient.AccessManagerAsyncClientBase class.
    /// </summary>
    public class AccessManagerAsyncClientBaseTests
    {
        ...
        [Test]
        public void HandleNonSuccessResponseStatusAsync_400StatusCodeWithHttpErrorResponseBody()
        {
        ...
        [Test]
        public void HandleNonSuccessResponseStatusAsync_500StatusCodeWithHttpErrorResponseBody()
        {
        ...
        [Test]
        public void HandleNonSuccessResponseStatusAsync_UnhandledStatusCodeWithHttpErrorResponseBody()
        {
            var testUrl = new Uri(baseUrl, "userToGroupMappings/user/user1/group/group1");
        ...
        [Test]
        public void HandleNonSuccessResponseStatusAsync_NonJsonResponseBody()
        {
            var testUrl = new Uri(baseUrl, "userToGroupMappings/user/user1/group/group1");
            const String errorMessage = "The server is not ready to handle the request.";
            using (var responseBody = new MemoryStream())
        ...
        [Test]
        public void HandleNonSuccessResponseStatusAsync_WhitespaceResponseBody()
        {
            var testUrl = new Uri(baseUrl, "userToGroupMappings/user/user1/group/group1");
            using (var responseBody = new MemoryStream())
            {
        ...
        private class TestAccessManagerAsyncClientWithProtectedMembers<TUser, TGroup, TComponent, TAccess> : AccessManagerAsyncClientBase<TUser, TGroup, TComponent, TAccess>
        {
            ...
            /// <summary>
            /// Handles receipt of a non-success HTTP response status, by converting the status and response body to an appropriate Exception and throwing that Exception as an asyncronous operation.
            /// </summary>
            ...
            public async new Task HandleNonSuccessResponseStatusAsync(HttpMethod method, Uri requestUrl, HttpStatusCode responseStatus, Stream responseBody)
            {
                await base.HandleNonSuccessResponseStatusAsync(method, requestUrl, responseStatus, responseBody);
            }

            /// <summary>
            /// Attempts to deserialize the body of a HTTP response received as a <see cref="Stream"/> to an <see cref="HttpErrorResponse"/> instance as an asyncronous operation.
            /// </summary>
            ...
            public async new Task<HttpErrorResponse> DeserializeResponseBodyToHttpErrorResponse(Stream responseBody)
            {
                return await base.DeserializeResponseBodyToHttpErrorResponseAsync(responseBody);
            }
        }
