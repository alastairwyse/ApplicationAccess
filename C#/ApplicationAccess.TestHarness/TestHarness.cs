...
namespace ApplicationAccess.TestHarness
{
    ...
    public class TestHarness<TUser, TGroup, TComponent, TAccess> : IDisposable
    {
        ...
        protected IList<IAccessManager<TUser, TGroup, TComponent, TAccess>> testAccessManagers;
        ...
        /// <param name="testAccessManagers">The access manager to test (the same should be assigned to each element), or the client instances which interface to the remote access manager to test.</param>
        ...
        (
            ...
            IList<IAccessManager<TUser, TGroup, TComponent, TAccess>> testAccessManagers,
        ...
        )
        {
            ...
            if (testAccessManagers.Count != workerThreadCount)
                throw new ArgumentException($"Parameter '{nameof(testAccessManagers)}' is expected contain the same number of elements {workerThreadCount} as in parameter '{nameof(workerThreadCount)}' but contained {operationGenerators.Count}.", nameof(testAccessManagers));
            ...
            this.testAccessManagers = testAccessManagers;
        ...
        public void Start()
        {
            ...
            for (Int32 i = 0; i < workerThreadCount; i++)
            {
                ...
                {
                    WorkerThreadRoutine(testAccessManagers[arrayIndex], operationGenerators[arrayIndex], parameterGenerators[arrayIndex], operationTriggerers[arrayIndex], exceptionLoggers[arrayIndex]); }
                );
        ...
        )
        {
            ...
            while (stopMethodCalled == false)
            {
                ...
                if (operationLimit > 0 && operationCount >= operationLimit && stopMethodCalled == false)
                {
                    Console.WriteLine($"Stopping testing as operation limit of {operationLimit} has been reached.");
                    Stop();
                }
